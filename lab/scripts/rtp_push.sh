...
resource=/opt/application/tx-rtcStream/files/resources
...
## （简化后）RTP视频传输
ffmpeg -re -protocol_whitelist file -i /opt/application/tx-rtcStream/files/resources/20_input.mp4 -filter_complex '[0:v]boxblur=3:1[v0]' -map [v0] -c:v vp8 -b:v 1000k  -ssrc 2222 -payload_type 101 -f rtp  rtp://121.5.133.154:10022
...
## 推送 ts 文件到 rtp
ffmpeg -re -protocol_whitelist file -i /opt/application/tx-rtcStream/files/synthesis/output.ts -filter_complex '[0:v]boxblur=3:1[v0]' -map [v0] -c:v vp8 -b:v 1000k  -ssrc 2222 -payload_type 101 -f rtp  rtp://121.5.133.154:10080


ffmpeg -re -protocol_whitelist file -i input.mp4 -c:v vp8 -b:v 1000k  -ssrc 2222 -payload_type 101 -f rtp  rtp://121.5.133.154:10016
...
ffmpeg -re -i /opt/application/tx-rtcStream/files/resources/20_input.mp4 -c copy -bsf:v h264_mp4toannexb -f mpegts output_file.ts
